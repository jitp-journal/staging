...
If you open either template, you'll see guidance for which styles apply to which elements of a typical JITP document. You'll need this for step 1c.

### 1b. Attach the templates.
...
Follow the instructions described in the template doc you downloaded to know which style applies where. You'll want to have the Styles pane open here, too, so you can quickly see what styles have been applied.
...
If you're running Mammoth on your own computer, carry on to step 2. If not, send the templated .docx file to the <abbr title="Editorial Collective">EC</abbr> member who will run it and update the staging spreadsheet so the Managing Editor knows where we are in the process. If you're not sure who that person is, send it to the Managing Editor.
